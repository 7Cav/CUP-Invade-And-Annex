[] spawn {
	...
	player addAction ["<t color='#00FF00'>View distance settings</t>", CHVD_fnc_openDialog, [], -99, false, true, "", "CHVD_enabled"];

	player addEventHandler ["Respawn", { player addAction ["<t color='#00FF00'>View distance settings</t>", CHVD_fnc_openDialog, [], -99, false, true, "", "CHVD_enabled"] }];

	//Detect when to change setting type
	[] spawn
	{
		scriptName "CHVD_fnc_init 1";
		for "_i" from 0 to 1 step 0 do
		{
			[nil, false] call CHVD_fnc_updateSettings;
			_currentVehicle = vehicle player;
			waitUntil { sleep 0.1; _currentVehicle != vehicle player};
		};
	};
	[] spawn
	{
		scriptName "CHVD_fnc_init 2";
		for "_i" from 0 to 1 step 0 do
		{
			waitUntil { sleep 0.1; UAVControl (getConnectedUAV player) select 1 != ""};
