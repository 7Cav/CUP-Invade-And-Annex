...
fnc_deploy_vehicle = {
	...
	if (((_randSafePos select 0) != (worldSize/2)) or ((_randSafePos select 1) != (worldSize/2))) then
	{
...
_redforSquadCount = 2 * ceil(_playerCount / (count redfor_squad_deploymentData));
_redforVehicleCount = ceil(_playerCount / (count redfor_squad_deploymentData));
_redforVehicleLevels = [redfor_vehicle_level_0_deploymentData];
...
_redforDeployedVehicleLevels = [redfor_vehicle_level_0_deploymentData];
...
if (redfor_vehicleLevelScaling) then
{
	...
	_redforVehicleLevels = [
		[redfor_vehicle_level_1_deploymentData, 1]
